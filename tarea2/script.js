import * as d3 from "https://cdn.jsdelivr.net/npm/d3@7/+esm"

const cities = [
    {name:"Amieirinha",population:4812946},
    {name:"Kinshasa",population:1027499},
    {name:"Blantyre",population:1992831},
    {name:"Pueblo Nuevo Viñas",population:6106658},
    {name:"Ko Si Chang",population:1258350},
    {name:"Rabak",population:5611054},
    {name:"Port-Cartier",population:2014142},
    {name:"Detroit",population:8927289},
    {name:"Medeiros Neto",population:6847563},
    {name:"Kushchëvskaya",population:4160962}

]

const svgWidth = 600
const svgHeight = 300

// escalas para el alto de las barras
const xScale = d3.scaleBand()
    .domain(cities.map(d => d.name))
    .range([0, svgWidth - 100]) // un poco de margen
    .padding(0.2)

const yScale = d3.scaleLinear()
    .domain([0, d3.max(cities, d => d.population)])
    .range([0, svgHeight - 50]) // margen arriba

// dibujar barras
d3.select('.bars')
    .selectAll('rect')
    .data(cities)
    .join('rect')
    .attr('x', d => xScale(d.name))
    .attr('y', d => svgHeight - yScale(d.population)) 
    .attr('width', xScale.bandwidth())
    .attr('height', d => yScale(d.population))

// textos abajo de cada barra
d3.select('.labels')
    .selectAll('text')
    .data(cities)
    .join('text')
    .attr('x', d => xScale(d.name) + xScale.bandwidth()/2)
    .attr('y', svgHeight - 5)
    .attr('text-anchor', 'end')  // se usa 'end' para que quede bien al rotar
    .attr('transform', d => {
        return `rotate(-45, ${xScale(d.name) + xScale.bandwidth()/2}, ${svgHeight - 5})`
    })
    .text(d => d.name)

/*
d3.select('.bars')
    .selectAll('rect')
    .data(cities)
    .join('rect')
    .attr('height', 19)
    .attr('width', function(d){
        return d.population * 40e-6
    })
    .attr('y', function(d, i){
        return i * 20
    })

d3.select('.labels')
    .selectAll('text')
    .data(cities)
    .join('text')
    .attr('y', function(d, i){
        return i * 20 + 13
    })
    .text(function(d){
        return d.name
    })
*/
